...
import { X, Send, Search, User as UserIcon, Sparkles, MessageCircle, CheckCheck } from 'lucide-react';
...
export const MessagesModal: React.FC = () => {
  ...
  // Get list of potential message contacts (followers, following, or existing chat history)
  const contacts = React.useMemo(() => {
    if (!currentUser) return [];
    const contactIds = new Set<string>();

    // Add users who have messaged with currentUser
    messages.forEach((m) => {
      if (m.senderId === currentUser.id) contactIds.add(m.receiverId);
      if (m.receiverId === currentUser.id) contactIds.add(m.senderId);
    });

    // Add followed and follower users
  ...
  // Get conversation messages between currentUser and selectedUser
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl h-[85vh] max-h-[700px] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {selectedUser ? (
            ...
            <div className="hidden sm:flex flex-1 items-center justify-center p-6 text-slate-400 text-xs">
              Sohbet başlatmak için soldaki listeden bir kullanıcı seçin.
            </div>
          )}
